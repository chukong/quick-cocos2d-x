    ...
    int       nCmdShow)
{
    ...
    return CocopushPlayer::createAndRun();
}

int CocopushPlayer::createAndRun(void)
{
    CocopushPlayer *host = CocopushPlayer::sharedInstance();
    int ret = host->run();
    CocopushPlayer::purgeSharedInstance();
    return ret;
}

CocopushPlayer *CocopushPlayer::s_sharedInstance = NULL;

CocopushPlayer *CocopushPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new CocopushPlayer();
    }
    return s_sharedInstance;
}

void CocopushPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
CocopushPlayer::CocopushPlayer(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int CocopushPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "cocopush-player");
...
void CocopushPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void CocopushPlayer::createViewMenu(void)
{
...
void CocopushPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void CocopushPlayer::updateWindowTitle(void)
{
}

void CocopushPlayer::relaunch(void)
{
...
void CocopushPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void CocopushPlayer::onFileOpenProject(void)
{
...
void CocopushPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"CocopushPlayer");
...
void CocopushPlayer::onFileProjectConfig(void)
{
...
void CocopushPlayer::onFileRelaunch(void)
{
    relaunch();
}

void CocopushPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void CocopushPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void CocopushPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void CocopushPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void CocopushPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT CocopushPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    CocopushPlayer *host = CocopushPlayer::sharedInstance();
...
INT_PTR CALLBACK CocopushPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
