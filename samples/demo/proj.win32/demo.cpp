    ...
    int       nCmdShow)
{
    ...
    return DemoPlayer::createAndRun();
}

int DemoPlayer::createAndRun(void)
{
    DemoPlayer *host = DemoPlayer::sharedInstance();
    int ret = host->run();
    DemoPlayer::purgeSharedInstance();
    return ret;
}

DemoPlayer *DemoPlayer::s_sharedInstance = NULL;

DemoPlayer *DemoPlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new DemoPlayer();
    }
    return s_sharedInstance;
}

void DemoPlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
int DemoPlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "demo-player");
...
void DemoPlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void DemoPlayer::createViewMenu(void)
{
...
void DemoPlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void DemoPlayer::updateWindowTitle(void)
{
}

void DemoPlayer::relaunch(void)
{
...
void DemoPlayer::writeDebugLog(const char *log)
{
...
// menu callback
void DemoPlayer::onFileOpenProject(void)
{
...
void DemoPlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"DemoPlayer");
...
void DemoPlayer::onFileProjectConfig(void)
{
...
void DemoPlayer::onFileRelaunch(void)
{
    relaunch();
}

void DemoPlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void DemoPlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void DemoPlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void DemoPlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void DemoPlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT DemoPlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    DemoPlayer *host = DemoPlayer::sharedInstance();
...
INT_PTR CALLBACK DemoPlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
