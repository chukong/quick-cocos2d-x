...
function DecorateBehavior:bind(object)
    ...
    local function updateView(object)
        ...
        local mapId        = object.map_:getId()

        local function updateView_(source)
            if not source then return end
            for decorationName, decoration in pairs(source) do
                if not decoration then
                    if decorationName == "ShipWavesUp" or decorationName == "ShipWaves" then
                        decoration = Decoration.new(decorationName, object.staticIndex_, mapId)
                    else
