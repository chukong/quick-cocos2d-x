...
function Store.init(listener)
    ...
    if store then
        echoError("[framework.api.Store] ERR, init() store already init")
        return false
    end

    if type(listener) ~= "function" then
        echoError("[framework.api.Store] ERR, init() invalid listener")
        return false
    end
...
function Store.getReceiptVerifyMode()
    ...
    if not store then
        echoError("[framework.api.Store] ERR, getReceiptVerifyMode() store not init")
        return false
    end
...
function Store.setReceiptVerifyMode(mode, isSandbox)
    ...
    if not store then
        echoError("[framework.api.Store] ERR, setReceiptVerifyMode() store not init")
        return false
    end
    ...
    if type(mode) ~= "number"
        ...
        echoError("[framework.api.Store] ERR, setReceiptVerifyMode() invalid mode")
        return false
    end
...
function Store.getReceiptVerifyServerUrl()
    ...
    if not store then
        echoError("[framework.api.Store] ERR, getReceiptVerifyServerUrl() store not init")
        return false
    end
...
function Store.setReceiptVerifyServerUrl(url)
    ...
    if not store then
        echoError("[framework.api.Store] ERR, setReceiptVerifyServerUrl() store not init")
        return false
    end

    if type(url) ~= "string" then
        echoError("[framework.api.Store] ERR, setReceiptVerifyServerUrl() invalid url")
        return false
    end
...
function Store.canMakePurchases()
    ...
    if not store then
        echoError("[framework.api.Store] ERR, canMakePurchases() store not init")
        return false
    end
...
function Store.loadProducts(productsId, listener)
    ...
    if not store then
        echoError("[framework.api.Store] ERR, loadProducts() store not init")
        return false
    end

    if type(listener) ~= "function" then
        echoError("[framework.api.Store] ERR, loadProducts() invalid listener")
        return false
    end

    if type(productsId) ~= "table" then
        echoError("[framework.api.Store] ERR, loadProducts() invalid productsId")
        return false
    end

    for i = 1, #productsId do
        if type(productsId[i]) ~= "string" then
            echoError("[framework.api.Store] ERR, loadProducts() invalid id[#%d] in productsId", i)
            return false
        end
    end
...
function Store.cancelLoadProducts()
    if not store then
        echoError("[framework.api.Store] ERR, cancelLoadProducts() store not init")
        return false
    end
...
function Store.isProductLoaded(productId)
    ...
    if not store then
        echoError("[framework.api.Store] ERR, isProductLoaded() store not init")
        return false
    end
...
function Store.purchase(productId)
    ...
    if not store then
        echoError("[framework.api.Store] ERR, purchase() store not init")
        return false
    end

    if type(productId) ~= "string" then
        echoError("[framework.api.Store] ERR, purchase() invalid productId")
        return false
    end
...
function Store.finishTransaction(transaction)
    ...
    if not store then
        echoError("[framework.api.Store] ERR, finishTransaction() store not init")
        return false
    end

    if type(transaction) ~= "table" or type(transaction.transactionIdentifier) ~= "string" then
        echoError("[framework.api.Store] ERR, finishTransaction() invalid transaction")
        return false
    end
